...
class NuevaExploracion: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UITextFieldDelegate {
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        ...
        if Float(textField.text!)!<0{
            aux=0.0
        }else if Float(textField.text!)!>1{
            aux=1.0
        }else{
            aux=Float(textField.text!)!
        }
        slider.value=aux
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        (cell as! NuevaExploracionCell).valorAtributoTxt.delegate=self
       // cell.valorAtributoTxt.delegate=self
    ...
    func setValorAtributo(valor: Int)
    {
        ...
        cell.setValorAtributo(with: Float(valor))
        
    }
    @IBAction func setValorAtributoSlider(_ sender: UISlider) {
        if cView.indexPathsForSelectedItems?.count == 0 {return}
        
        let cell = cView.cellForItem(at: cView.indexPathsForSelectedItems!.last!) as! NuevaExploracionCell
        cell.setValorAtributo(with: sender.value)
    }
}
